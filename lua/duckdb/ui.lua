...
function M.display_results(result, options)
  ...
  vim.api.nvim_set_option_value("filetype", "duckdb-result", { buf = buf })
  ...
  -- Setup result buffer actions (filter, sort, yank, export, etc.)
  local actions = require("duckdb.actions")
  actions.setup_keymaps(buf, {
    query = options.query,
    result = result,
