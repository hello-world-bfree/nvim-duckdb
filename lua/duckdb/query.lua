...
function M.load_buffer_data(connection, table_name, buffer_info)
	...
	-- vim.notify("format: " .. buffer_info.format, vim.log.levels.DEBUG)
	if buffer_info.format == "csv" then
		-- Escape single quotes in content
		local escaped_content = buffer_info.content:gsub("'", "''")
		-- vim.notify("escaped_content: " .. escaped_content, vim.log.levels.DEBUG)
		-- local buffer_data = vim.api.nvim_buf_get_lines(0, 0, -1, false)
		-- vim.notify("buffer_data: " .. table.concat(buffer_data, "\n"), vim.log.levels.DEBUG)
		vim.system({
			"sh",
			"-c",
		...
		-- vim.notify("create_query: " .. create_query, vim.log.levels.DEBUG)
	...
	local success, result, error_msg = pcall(function()
		...
		for i, id in ipairs(identifiers) do
			...
			if type(id) == "string" then
				...
				vim.notify("Replaced buffer('" .. id .. "') with " .. table_name, vim.log.levels.DEBUG)
			else
